...
public class Main180101 {

	public static void main(String[] args) {
        Scanner in = new Scanner(System.in);
        int t=in.nextInt();
        String [] movie=new String[t];
        for(int i=0;i<t;i++) {
        	movie[i]=in.next();
        }
        in.close();
        ...
        for(int i=0;i<t;i++) {
        	dis[i]=0;
        	int [] x=new int[movie[i].length()];
        	int [] y=new int[movie[i].length()];
        	int [] d=new int[movie[i].length()];
        	
        	for(int j=0;j<movie[i].length();j++) {
        		//char m=movie[i].charAt(j);
        		String m=movie[i].substring(j,j+1);
        		switch(m){
        		case "A":
        		case "B":
        		case "C":
...
//        		    break;
        		...
        		if(j==0) {
        			d[j]=getD(0,x[j],0,y[j]);
        		}else {
        			d[j]=getD(x[j-1],x[j],y[j-1],y[j]);
        		}
        		dis[i]+=d[j];
	...
	public static int getD(int x0,int x1,int y0,int y1){
		int x=(x0-x1);
		x=x>0?x:-x;
		int y=(y0-y1);
		y=y>0?y:-y;
